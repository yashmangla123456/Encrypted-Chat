...
var chat = {}; // socket.io
// var selected_user_lst = selected_users;
...
module.exports = function (app, io) {
  ...
  app.get("/getname/:name", (req, res) => {
    ...
    res.send({ id: socketIDs[req.params.name] });
  });
    ...
    socket.on("login", (data) => {
      ...
      if (user) {
        ...
        if (user.password == userHashedPass) {
          ...
          // exist user, entry password is incorrect
...
function userSigned(user, socket) {
  ...
  socketIDs[user.username] = user.socketid;
} // signed-in

//api to return socketID
...
function defineSocketEvents(socket) {
  ...
  // Handle the sending of messages to specific users
  socket.on("special-msg", (data) => {
  ...
  socket.on("fetch-special-messages", (channelName, u_users) => {
    // find fetcher user
    ...
    if (
      ...
      if (u_users.indexOf(fetcher.id) !== -1)
        socket.emit("fetch-special-messages", {
          channel: channel.name,
